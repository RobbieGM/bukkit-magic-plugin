	...
	public boolean use(ItemStack wandItem, Player player, World world, Server server) {
		for (int tickDelay = 0; tickDelay <= 9; tickDelay += 3) {
			server.getScheduler().scheduleSyncDelayedTask(plugin, () -> {
				Location loc = player.getEyeLocation().toVector().add(player.getLocation().getDirection().multiply(0.5))
						.toLocation(world, player.getLocation().getYaw(), player.getLocation().getPitch());
				Arrow arrow = (Arrow) world.spawnArrow(loc, loc.getDirection(), 1.8f, isBuffed ? 4 : 6);
				if (isBuffed)
